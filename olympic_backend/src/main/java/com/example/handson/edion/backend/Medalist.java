package com.example.handson.edion.backend;
...
public class Medalist {
    ...
    private static final Logger logger = Logger.getLogger(Medalist.class.getName());
    ...
    public String getMedalist(@Context HttpHeaders headers) {
        ...
        logger.log(Level.INFO, "Service Version: {0}", SERVICE_VERSION);
        ...
        logger.log(Level.INFO, "backend method end: {0}", Thread.currentThread().getStackTrace()[1].getMethodName());
    ...
    private WebTarget getURL(String serviceURL) {
        ...
        logger.log(Level.INFO, "Service URL: {0}", target.toString());
        return target;
    }
